...
function themencode_theme_enqueue_styles() { 
    wp_enqueue_style( 'parent-style', get_template_directory_uri() . '/style.css' );  
}
add_action( 'wp_enqueue_scripts', 'themencode_theme_enqueue_styles' );
...
add_action( 'init', 'pw_edd_comments', 99999);

/**
 * WordPress Breadcrumbs
 */
function thmn_wp_custom_breadcrumbs() {

    $separator              = '/';
    $breadcrumbs_id         = 'thmn_breadcrumbs';
    $breadcrumbs_class      = 'thmn_breadcrumbs';
    $home_title             = esc_html__('Home', 'your-domain');

    // Add here you custom post taxonomies
    $thmn_custom_taxonomy    = 'product_cat';
    ...
    if ( !is_front_page() ) {
        ...
        if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
            ...
            $taxonomy_exists = taxonomy_exists($thmn_custom_taxonomy);
            if(empty($last_category) && !empty($thmn_custom_taxonomy) && $taxonomy_exists) {
                   
                $taxonomy_terms = get_the_terms( $post->ID, $thmn_custom_taxonomy );
                $cat_id         = $taxonomy_terms[0]->term_id;
                $cat_nicename   = $taxonomy_terms[0]->slug;
                $cat_link       = get_term_link($taxonomy_terms[0]->term_id, $thmn_custom_taxonomy);
